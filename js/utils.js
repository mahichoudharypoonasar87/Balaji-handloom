...
// ─── PRODUCT IMAGE HTML (with "No Image" fallback) ──────────────────────────
/**
 * Returns an <img> tag if a usable image URL exists, otherwise returns a
 * styled "No Image" placeholder block. Use anywhere a product image is shown
 * (product cards, galleries, admin tables) instead of falling back to a
 * placeholder file that may not exist on disk.
 */
export function productImageHtml(src, alt = "", className = "", extraAttrs = "") {
  if (src) {
    return `<img src="${src}" alt="${alt}" class="${className}" loading="lazy" ${extraAttrs} />`;
  }
  return `
    <div class="no-image-placeholder ${className}">
