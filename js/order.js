      ...
      <div class="glass-card" style="padding:var(--space-6);">
        ...
        ${(order.items || []).map((item) => `
          <div style="display:flex; gap:var(--space-4); padding:var(--space-4) 0; border-bottom:1px solid var(--clr-border);">
            ${item.image ? `<img src="${item.image}" alt="${item.name}" style="width:64px;height:80px;object-fit:cover;border-radius:var(--radius-md);" loading="lazy" />` : `<div style="width:64px;height:80px;border-radius:var(--radius-md);overflow:hidden;">${productImageHtml("", item.name)}</div>`}
            <div style="flex:1;">
